		...
		$.ajax({
			...
			success : function(result) {
				...
				$.each(result.map.pageInfo.list,
					...
					//构建节点
					var wordsCheckbox = $("<td></td>")
					.append(
					"<input type='checkbox' class='words_checkbox' />")
					var wordsId = $("<td></td>").append(
							item.empname);
					var wordsName = $("<td> </td>")
					.append(item.content);
					var wordsDesc = $("<td> </td>")
					...
					.append(
							...
							.append("编辑");
					// 把id和name放入按钮中
					bj_Btn.attr("words_id", item.wordsId)
					bj_Btn.attr("words_name", item.wordsName)
					...
					$("<tr></tr>").append(wordsCheckbox)
					.append(wordsId).append(
							wordsName).append(
									wordsDesc).append(
											btn_td).appendTo(
											"#tab_body");
