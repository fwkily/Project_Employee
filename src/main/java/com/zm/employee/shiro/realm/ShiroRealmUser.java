...
*/
public class ShiroRealmUser extends AuthorizingRealm{
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		System.out.println("ShiroRealmUser 从数据库中查询：" + username + " 的信息");
		...
		try {
			//管理员是用字符串登陆  如果这里不能转换成功  说明输入的是员工的ID  直接返回null
			criteria.andEmpIdEqualTo(Integer.parseInt(username));
		} catch (NumberFormatException e) {
			// TODO: handle exception
			return null;
		}
		List<Employee> principal =  mapper.selectByExampleAndDept(example);
		String credentials ="";
...
//		Employee principal = (Employee) principals.getPrimaryPrincipal();
